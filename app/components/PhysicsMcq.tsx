...
import { PHYSICS_BATCHES } from "./physics/data";
import {
  PHYSICS_GAME_STORAGE_KEY,
  readStoredGameProgress,
  STARTING_HEARTS,
  toStoredGameProgress
} from "./physics/utils/gameProgress";
import { type RandomQuestion } from "./physics/types";
...
export default function PhysicsMcq() {
  ...
  useEffect(() => {
    ...
    safeWriteStorageJson(PHYSICS_GAME_STORAGE_KEY, toStore);
  }, [
    hearts,
    currentStreak,
    bestStreak,
  ...
  return (
    <>
      <McqSubjectCard
        icon="🔭"
        title="பெளதிகவியல் (Physics)"
        meta={`11 தொகுதிகள் · ${totalExamCount} பரீட்சைகள் · இலக்கு: ${totalTarget} கேள்விகள்`}
