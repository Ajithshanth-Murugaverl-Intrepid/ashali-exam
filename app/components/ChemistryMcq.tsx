...
import { CHEMISTRY_BATCHES } from "./chemistry/data";
import {
  CHEMISTRY_GAME_STORAGE_KEY,
  readStoredGameProgress,
  STARTING_HEARTS,
  toStoredGameProgress
} from "./chemistry/utils/gameProgress";
...
export default function ChemistryMcq() {
  ...
  const totalTarget = useMemo(
    () => CHEMISTRY_BATCHES.reduce((sum, batch) => sum + batch.exams.reduce((inner, exam) => inner + exam.target, 0), 0),
    []
  );
  const totalExamCount = useMemo(() => CHEMISTRY_BATCHES.reduce((sum, batch) => sum + batch.exams.length, 0), []);
  ...
  const selectedBatch = useMemo(() => CHEMISTRY_BATCHES.find((batch) => batch.id === selectedBatchId) ?? null, [selectedBatchId]);

  const selectedExam = useMemo(() => {
    if (!selectedExamKey) return null;
    const [batchId, examCode] = selectedExamKey.split(":");
    const batch = CHEMISTRY_BATCHES.find((item) => item.id === batchId);
  ...
  useEffect(() => {
    ...
    safeWriteStorageJson(CHEMISTRY_GAME_STORAGE_KEY, toStore);
  }, [
    hearts,
    currentStreak,
    bestStreak,
  ...
  return (
    <>
      <McqSubjectCard
        icon="⚗️"
        title="இரசாயனவியல் (Chemistry)"
        meta={`${CHEMISTRY_BATCHES.length} தொகுதிகள் · ${totalExamCount} பரீட்சைகள் · இலக்கு: ${totalTarget} கேள்விகள்`}
        ...
        >
          <div className="phy-modal">
            <div className="phy-modal-header">
              <div>
                <h2 className="phy-modal-title">⚗️ இரசாயனவியல் MCQ பயிற்சி</h2>
      ...
      {selectedExam && (() => {
        ...
        return (
          <ExamModule
            config={{
              title: `⚗️ இரசாயனவியல் தேர்வு — ${batch.label} (${exam.label})`,
              subject: "Chemistry",
              batchId: batch.id,
